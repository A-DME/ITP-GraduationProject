...
// MARK: - Todo: Put current customer's id
        networkManager?.fetch(url: APIHandler.urlForGetting(.allAddressesOf(customer_id: String(userDefault?.getCustomerId() ?? 0))), type: Addresses.self, complitionHandler: { result in
