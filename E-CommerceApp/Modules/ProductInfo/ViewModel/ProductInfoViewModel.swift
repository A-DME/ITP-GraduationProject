...
class ProductInfoViewModel{
    ...
    func loadData(productId:Int){
        ...
        networkHandler?.fetch(url: APIHandler.urlForGetting(.draftOrder(id: self.userDefaults?.getWishlistID() ?? "")), type: DraftOrderContainer.self, complitionHandler: { container in
            self.currentWishList = container?.draftOrder.lineItems
            print("wishList is here: \(self.userDefaults?.getWishlistID() ?? "")")
        })
        
        networkHandler?.fetch(url: APIHandler.urlForGetting(.draftOrder(id: self.userDefaults?.getCartID() ?? "")), type: DraftOrderContainer.self, complitionHandler: { container in
